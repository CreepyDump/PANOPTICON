...
//	dat += "<a href='?_src_=prefs;preference=tab;tab=3' [current_tab == 3 ? "class='linkOn'" : ""]>Keybinds</a>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			used_title = "Victim of Panopticon"
...
//			dat += "<a href='?_src_=prefs;preference=toggle_random;random_type=[RANDOM_NAME_ANTAG]'>When Antagonist: [(randomise[RANDOM_NAME_ANTAG]) ? "Yes" : "No"]</a>"
			...
			dat += "<BR>"
			dat += "<b>Race:</b> <a href='?_src_=prefs;preference=species;task=input'>[pref_species.name]</a><BR>"
...
//			dat += "<b><a href='?_src_=prefs;preference=name;task=random'>Random Name</A></b><BR>"
			dat += "<b>Faults:</b> <a href='?_src_=prefs;preference=charflaw;task=input'>[charflaw]</a><BR>"
...
/datum/preferences/proc/SetChoices(mob/user, limit = 10, list/splitJobs = list("Slug", "Necroleninist", "Major"), widthPerColumn = 295, height = 620) //295 620
	if(!SSjob)
		return
...
//		HTML += "<center><br><a href='?_src_=prefs;preference=job;task=random'>[message]</a></center>"
	...
	var/datum/browser/popup = new(user, "mob_occupation", "<div align='center'>Role Selection</div>", width, height)
	popup.set_window_options("can_close=0")
	popup.set_content(HTML)
	popup.open(FALSE)
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("age")
					var/new_age = input(user, "Choose your character's age (18-[pref_species.max_age])", "Character Preference") as null|anything in pref_species.possible_ages
					if(new_age)
						age = new_age
						var/list/hairs
						if((age == AGE_OLD) && (OLDGREY in pref_species.species_traits))
...
//сука я заебался
				...
				if("s_tone")
					...
					var/new_s_tone = input(user, "Choose your character's skin tone:", "Character Preference")  as null|anything in listy
					if(new_s_tone)
						...
						if(S)
							for(var/i=1, i<=max_save_slots, i++)
								...
								if(!name)
									name = "Slot[i]"
								choices[name] = i
					var/choice = input(user, "CHOOSE","PANOPTICON") as null|anything in choices
					if(choice)
...
//	character.accessory = accessory
	...
	if(charflaw)
		...
		if(istype(charflaw, /datum/charflaw/faggot))
