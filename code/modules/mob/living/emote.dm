...
/datum/emote/living/stare/adjacentaction(mob/user, mob/target)
	. = ..()
	if(!user || !target)
		return
	if(ishuman(target))
		...
		H.flash_fullscreen("redflash3")
		H.AdjustSleeping(-50)
		playsound(user.loc, 'sound/misc/stare.ogg', 100, FALSE, -1)
...
/datum/emote/living/wipe/run_emote(mob/user, params, type_override, intentional)
	...
	if(HAS_TRAIT(user, TRAIT_WEARING_GAS_MASK))
		...
		playsound(user.loc, 'sound/panopticon/gasmask_wipe.ogg', 1, TRUE)
...
/mob/living/carbon/human/verb/emote_wipe()
	set name = "Wipe"
	set category = "Emotes"

	emote("wipe", intentional = TRUE)
