...
/datum/species/proc/regenerate_organs(mob/living/carbon/C,datum/species/old_species,replace_current=TRUE)
	...
	var/obj/item/organ/penis/penis = C.getorganslot(ORGAN_SLOT_PENIS)
	var/obj/item/organ/boobs/boobs = C.getorganslot(ORGAN_SLOT_BOOBS)
	...
	if(C.gender == FEMALE)
		if(prob(2))
			if(penis && replace_current)
				penis.Remove(C,1)
			QDEL_NULL(penis)
			if(!penis)
				penis = new()
				penis.Insert(C)
		if(boobs && replace_current)
			boobs.Remove(C,1)
			QDEL_NULL(boobs)
		if(!boobs)
			boobs = new()
