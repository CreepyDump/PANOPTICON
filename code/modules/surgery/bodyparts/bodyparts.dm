...
/obj/item/bodypart/attackby(obj/item/W, mob/user, params)
	if(W.get_sharpness())
		...
		return ..()
		if(!meat_type)
			to_chat(user, "<span class='warning'>\The [src] cannot be butchered for any meat.</span>")
			return
		...
		if(do_after(user, 54, target = src))
			...
			playsound(get_turf(src), 'sound/misc/fleisch.ogg', 100, FALSE, FALSE)
...
/obj/item/bodypart/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(status != BODYPART_ROBOTIC)
		playsound(get_turf(src), 'sound/misc/fleisch.ogg', 50, TRUE, -1)
	pixel_x = rand(-3, 3)
	pixel_y = rand(-3, 3)
	if(!skeletonized)
