...
/obj/item/gun/ballistic/automatic/proc/burst_select()
	...
	if(!select)
		burst_size = 1
		fire_delay = 0
		to_chat(user, "<span class='notice'>I switch to semi-automatic.</span>")
	else
		burst_size = initial(burst_size)
		fire_delay = initial(fire_delay)
		to_chat(user, "<span class='notice'>I switch to [burst_size]-rnd burst.</span>")
...
/obj/item/gun/ballistic/automatic/m90/burst_select()
	...
	switch(select)
		...
		if(1)
			select = 2
			to_chat(user, "<span class='notice'>I switch to grenades.</span>")
		if(2)
			select = 0
			burst_size = 1
			fire_delay = 0
			to_chat(user, "<span class='notice'>I switch to semi-auto.</span>")
	playsound(user, 'sound/blank.ogg', 100, TRUE)
	update_icon()
	return
...
/obj/item/gun/ballistic/automatic/panopticon/krasnik
	name = "Krasnikov Automatic Rifle - 37"
	desc = ""
	icon = 'icons/panopticon/items/64.dmi'
	icon_state = "krasnik"
	item_state = "krasnik"
...
/obj/item/gun/ballistic/automatic/panopticon/krasnik/getonmobprop(tag)
	...
	if(tag)
		switch(tag)
			if("gen")
				return list("shrink" = 0.4,"sx" = 5,"sy" = -3,"nx" = -5,"ny" = -2,"wx" = -5,"wy" = -1,"ex" = 3,"ey" = -2,"northabove" = 0,"southabove" = 1,"eastabove" = 1,"westabove" = 0,"nturn" = 7,"sturn" = -7,"wturn" = 16,"eturn" = -22,"nflip" = 8,"sflip" = 0,"wflip" = 8,"eflip" = 0,)
			if("wielded")
