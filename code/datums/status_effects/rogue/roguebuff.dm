...
/datum/status_effect/buff/heroin
	id = "heroin"
	alert_type = /atom/movable/screen/alert/status_effect/buff/heroin
	effectedstats = list("endurance" = 3,"speed" = -1,"constitution" = 3)
	duration = 10 SECONDS

/datum/status_effect/buff/heroin/on_apply()
	. = ..()
	owner.add_stress(/datum/stressevent/high)
	if(owner?.client)
		if(owner.client.screen && owner.client.screen.len)
...
/datum/status_effect/buff/heroin/on_remove()
	owner.remove_stress(/datum/stressevent/high)
	if(owner?.client)
		if(owner.client.screen && owner.client.screen.len)
			var/atom/movable/screen/plane_master/game_world/PM = locate(/atom/movable/screen/plane_master/game_world) in owner.client.screen
...
/atom/movable/screen/alert/status_effect/buff/heroin
	name = "High"
	desc = ""
	icon_state = "acid"
