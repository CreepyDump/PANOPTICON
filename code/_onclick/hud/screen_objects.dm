...
/atom/movable/screen/act_intent/rogintent/update_icon(var/list/intentsl,var/list/intentsr, oactive = FALSE)
	..()
	cut_overlays(TRUE)
...
/atom/movable/screen/act_intent/rogintent/proc/get_index_at_loc(var/xl, var/yl)
/*	if(xl<=64)
		if(xl<32)
			if(yl>16)
				return 1
...
/atom/movable/screen/zone_sel/update_overlays()
	...
	icon_state = "[hud.mymob.gender == "male" ? "m" : "m"]-zone_sel"
	...
	. += mutable_appearance(overlay_icon, "[hud.mymob.gender == "male" ? "m" : "m"]_[hud.mymob.zone_selected]")
...
/atom/movable/screen/stress/update_icon()
	...
	if(ishuman(usr))
		...
		if(H.has_status_effect(/datum/status_effect/buff/druqks))
			state2use = "mood_high"
		if(H.has_status_effect(/datum/reagent/psychick/on_mob_metabolize))
			state2use = "mood_high"
		if(H.InFullCritical())
			state2use = "mood_fear"
		if(H.mind)
...
atom/movable/screen/char_preview/Click()
	winset(usr.client, "character_preview_map", pick(prevcolors))
