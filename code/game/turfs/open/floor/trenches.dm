...
/turf/floor/trenches/relativewall()
	var/junction = 0
	for(var/turf/floor/trenches/W in orange(src,1))
		if(abs(src.x-W.x)-abs(src.y-W.y))
			junction |= get_dir(src,W)
	icon_state = "wood[junction]"

/turf/floor/trenches/ex_act()//No blowing this up.
	return


/turf/floor/trenches/underground
	icon = 'icons/obj/warfare.dmi'
	icon_state = "trench_old"
	is_underground = TRUE
	color = "#b5acbcff"

/turf/floor/trenches/underground/Initialize()
	dir = pick(GLOB.alldirs)

/turf/floor/trenches/underground/relativewall()
	return

/turf/floor/trenches/underground/relativewall_neighbours()
	return

/turf/floor/trenches/Initialize()
	. = ..()
...
/turf/floor/trench/fake
	atom_flags = null
	can_be_dug = FALSE

/turf/floor/trench/tough
	can_be_dug = FALSE

/turf/floor/trench/ex_act(severity)
	return

/turf/floor/trench/update_dirt()
...
/turf/floor/trench/RightClick(mob/living/user)
...
/turf/floor/proc/update_trench_layers()
	vis_contents.Cut()
	for(var/direction in GLOB.cardinal)
		var/turf/turf_to_check = get_step(src,direction)
		if(istype(turf_to_check, /turf/floor/trench))
			continue
		if(istype(turf_to_check, /turf/space) || istype(turf_to_check, /turf/floor) || istype(turf_to_check, /turf/floor/exoplanet/water/shallow) || istype(turf_to_check, /turf/wall))
...
/turf/floor/trench/update_icon()
	update_trench_shit()

/turf/floor/proc/update_trench_shit()
	for(var/direction in GLOB.cardinal)
		var/turf/turf_to_check = get_step(src,direction)
		if(istype(turf_to_check, /turf/floor/trench))//Rebuild our neighbors.
			var/turf/floor/trench/T = turf_to_check
...
/obj/structure/bridge/Process() // If turfs near bridge change - drop bridge
	...
	if(src.dir == NORTH || src.dir == SOUTH)
		...
		if(!istype(NORTH_dirt, /turf/floor/dirty) || !istype(SOUTH_dirt, /turf/floor/dirty))
			new /obj/item/bridge(src.loc)
			qdel(src)
			return
		...
		if(!istype(WEST_dirt, /turf/floor/dirty) || !istype(EAST_dirt, /turf/floor/dirty))
			new /obj/item/bridge(src.loc)
			qdel(src)
			return
...
/turf/floor/trench/attackby(obj/O as obj, mob/living/user as mob)
	if(istype(O, /obj/item/bridge))
		if(!user.doing_something)
			...
			if(do_after(user, (backwards_skill_scale(user.SKILL_LEVEL(engineering)) * 5)))
				...
				for(var/direction in GLOB.cardinal)
					var/turf/turf_to_check = get_step(src,direction)
					if(istype(turf_to_check, /turf/floor/dirty))
						turf_check++
...
/turf/floor/trench/Crossed(var/io)
	if(ishuman(io))
		...
		if(!M.throwing)
			...
			for(var/direction in GLOB.cardinal)
				...
				if(istype(turf_to_check, /turf/floor/trench))//And if they're a trench, count it.
					trench_check++
...
/turf/floor/trench/Uncrossed(var/io)
	if(ishuman(io))
		var/mob/living/carbon/human/M = io
		//if(M.client)
