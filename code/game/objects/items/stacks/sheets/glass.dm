...
/obj/item/stack/sheet/plasmaglass/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/rods))
		...
		if (V.get_amount() >= 1 && get_amount() >= 1)
			...
			to_chat(user, "<span class='warning'>I need one rod and one sheet of plasma glass to make reinforced plasma glass!</span>")
			return
	else
		return ..()
...
/obj/item/stack/sheet/plasmarglass
	...
	armor = list("blunt" = 20, "slash" = 20, "stab" = 20, "bullet" = 0, "laser" = 0, "energy" = 0, "bomb" = 0, "bio" = 0, "rad" = 0, "fire" = 80, "acid" = 100)
	resistance_flags = ACID_PROOF
	material_flags = MATERIAL_NO_EFFECTS
...
/obj/item/shard
	...
	resistance_flags = ACID_PROOF
	armor = list("blunt" = 100, "bullet" = 0, "laser" = 0, "energy" = 100, "bomb" = 0, "bio" = 0, "rad" = 0, "fire" = 50, "acid" = 100)
	max_integrity = 40
	sharpness = IS_SHARP
	var/icon_prefix
	...
	if(I.use_tool(src, user, 0, volume=50))
		...
		for(var/obj/item/stack/sheet/G in user.loc)
			...
			if(G.amount >= G.max_amount)
				continue
			G.attackby(NG, user)
		to_chat(user, "<span class='notice'>I add the newly-formed [NG.name] to the stack. It now contains [NG.amount] sheet\s.</span>")
		qdel(src)
	return TRUE
