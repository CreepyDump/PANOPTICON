...
/obj/structure/mirror/shitweb
	icon_state = "mirror_shitweb"
	pixel_y = 32
	...
	if(ishuman(user))
		...
		if(H.mob_timers["mirrortime"])
			if(world.time < H.mob_timers["mirrortime"] + 6 MINUTES)
				...
				var/chosen = input(user, "Change what?", "PANOPTICON")  as null|anything in options
				var/should_update
				switch(chosen)
					if("hairstyle")
					...
					if("facial hairstyle")
						...
						for(var/datum/sprite_accessory/X in spec_hair)
							hairlist += X.name
						var/new_hairstyle
						new_hairstyle = input(user, "Choose your character's beard:", "Character Preference")  as null|anything in hairlist
						if(new_hairstyle)
