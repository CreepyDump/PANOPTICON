...
/obj/structure/fluff/railing/wood/broken
	icon_state = "woodrailing-broken"
	blade_dulling = DULLING_BASHCHOP
	layer = ABOVE_MOB_LAYER
...
/obj/structure/bars/grille/Initialize()
	AddComponent(/datum/component/squeak, list('sound/foley/footsteps/FTMET_A1.ogg','sound/foley/footsteps/FTMET_A2.ogg','sound/foley/footsteps/FTMET_A3.ogg'), 100)
						...
						if(found_mobs.len == 2)
							...
							if(!index)
								...
								if(findtext(theman.real_name, " of ") || findtext(theman.real_name, " the "))
									surname2use = theman.dna.species.random_surname()
									theman.change_name(copytext_char(theman.real_name, 1,index))
								else
									surname2use = copytext_char(theman.real_name, index)
									theman.change_name(copytext_char(theman.real_name, 1,index))
							index = findtext(thewoman.real_name, " ")
							if(index)
								thewoman.change_name(copytext_char(thewoman.real_name, 1,index))
