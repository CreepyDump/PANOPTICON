...
/obj/effect/temp_visual/explosion/Initialize(mapload, radius, color = LIGHT_COLOR_LAVA, small = FALSE, large = FALSE)
	. = ..()
	...
	if(iswater(get_turf(src)))
		icon_state = null
		return
	var/image/img = image(icon, src, icon_state, 10, -32, -32)
	var/matrix/rotate = matrix()
	rotate.Turn(rand(0, 359))
	img.transform = rotate
	overlays += img //we use an overlay so the explosion and light source are both in the correct location plus so the particles don't rotate with the explosion
	icon_state = null
