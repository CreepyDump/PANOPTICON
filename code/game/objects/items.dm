...
/obj/item
	...
	var/throw_sound = 'sound/panopticon/throw.ogg'
...
//		owner.update_inv_ears()
	...
	if(H)
		...
		if(flags & ITEM_SLOT_HIP && (H.beltr == src || H.beltl == src) )
			owner.update_inv_belt()
		if(flags & SLOT_L_STORE && H.l_store == src)
			owner.update_inv_pockets()
		if(flags & SLOT_R_STORE && H.r_store == src)
			owner.update_inv_pockets()
	else
		if(flags & ITEM_SLOT_HEAD)
			owner.update_inv_head()
		...
		if(flags & ITEM_SLOT_BELT)
			owner.update_inv_belt()
		if(flags & SLOT_L_STORE)
			owner.update_inv_pockets()
		if(flags & SLOT_R_STORE)
			owner.update_inv_pockets()
